...
class Main:
    ...
    def __init__(self):
        ...
        # Quanto menor o ALFA, maior a influencia do feromonio
        # Quanto maior o BETA, menor a influencia heuristica
        ...
        #     print 'Amostra %1d' % i
        start_time = time.time()
        aco = Aco(gp.graph, 0.0014285714285714286, 0.5, 1.2, 0.5)
        aco_path, cost = aco.run(1000, args[0], args[1], 'weight')
        print 'ACO Menor caminho DISTANCIA(path, custo):', aco_path, cost, 'Run Time: ' + str(time.time()-start_time)

        # start_time = time.time()
        # aco = paco.Aco(gp.graph, 0.0014285714285714286, 0.5, 1.2, 0.5)
